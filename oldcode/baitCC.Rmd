...
#vaccine efficacy Gibson (2020) for Bangalore
Inj=1.00
Oral=0.8
...
# use this switch for either 1 vaccine coverage constraint on sum of SC and NC dogs, or 2, seperate coverage constraints on SC and NC dogs, or 3, seperate, but probabilistic constraints
iq=2

# percentile for prob constraints (note that 0.5 is the same as the mean)
probs=0.7
...
filled.contour(obs,NCseq, pdogcost, 
               plot.title = title(main = "",
    ylab = "% never confined dogs", xlab = "oral bait cost (USD)",
     abline(v=bkline),text(x=2.5, y=0.6,paste("oral baits no longer cost effective",sprintf('\u2192')))),
...
plot(Dsize,bkevm[1,], type="b", pch=16, cex=.8, las=1, bty="l",
     ylim=c(0, 5.), xlim=c(0, 123000),yaxt="n",lwd=2,
     ylab="Break even cost of oral bait (USD)",
xlab="Total size of dog population")
#  axis(2, at=c(0.2, 0.4, 0.6, 0.8), labels=c("0.2", "0.4", "0.6", "0.8"),las=1)
...
legend("bottomright", legend=NCseq[c(1,6,21,91)], bty="n", pch=16, lty=1, 
       col=c("black", "dark goldenrod", "blue", "pink3"), title="% NC dogs")
dev.off()
...
plot(NCseq, tdogs[10,], ylim=c(35000, 50000), type="l", lwd=2,las=1,bty="l",
     ylab="",cex=.9, cex.lab=1.2,
     xlab="percent of NC dogs in the population")
lines(NCseq, tdogs2[10,], lwd=2, lty=2)
lines(NCseq, tdogs7[10,], lwd=2, lty=3)
...
plot(NCseq, tcost[10,], 
     ylim=c(50000, 120000), 
     type="l", lwd=2,las=1,bty="l",
     ylab="",cex=.9, cex.lab=1.2,
     xlab="percent of NC dogs in the population")
lines(NCseq, tcost2[10,], lwd=2, lty=2)
lines(NCseq, tcost7[10,], lwd=2, lty=3)
title(ylab="partial campaign costs (USD)", line = 4, cex.lab=1.2)
legend("bottomright", legend=c(.20, .50, .70), title="prob of success", lty=c(2,1,3), bty="n")
```