...
peritem=c(costG[27:28,1], paste(costG[29,1], "(CP/DD/CVR)"), paste(costG[30,1], "(CP/DD)"), paste(costG[31,1], "(CVR/ORV)"),techs, "CVR driver","DD vehicle rental & fuel","CVR vehicle rental & fuel", "ORV vehicle rental & fuel",costG[24,1], costG[25,1], "ORV PEP (1 in 1,000)") 
...
```{r varycomply, eval=FALSE}
# use this setup to change vaccine accessibility 
#(change eval to TRUE here and at end of loop)
...
knitr::kable(pve, digits=2, caption="Table: Vaccination accessibility from Gibson et al. 2020")
...
```{r}
# set eval=TRUE to loop through dog population size
# and at line 512 and 650
# loop used to create fig 2 if desired, if not use single value for Dsize
...
sprlist=c(6,46)
spr=sprlist[1]
plot(NA, xaxt = "n", yaxt = "n", xlab="oral bait cost (USD)", ylab="per dog vaccination cost for optimal strategy (USD)",type="l", lwd=2,las=1, bty="L", xlim=c(0.5,4.1),ylim=c(0.1, 4.25))
...
#text(x=2.5, y=pdogcostNO[spr]+.2,"per dog cost in campaign without oral baits", col="dark blue")
legend("bottomleft",legend=NCseq[sprlist], bty="n", col=c("dark goldenrod","dark blue"), lty=1, title="proportion NC")
...
```{r totalcounter, eval=TRUE}
#eval =TRUE varying varying dog pop
if (idx==1){
  bkevs[jp]=NA
  }else{
bkevs[jp]=obs[idx-1]
...
# related figure to console
#bkline=min(obs[which(round(rowSums(M), 0)==0)]) need to detect sign change
#bkline=obs[diff(sign(rowSums(M)))!=0]?
par(mar=c(5,5,3,3))
filled.contour(obs,NCseq, pdogcost, 
               plot.title = title(main = "",
    ylab = "% never confined dogs", xlab = "oral bait cost (USD)",
     #abline(v=bkline),
    text(x=3.5, y=0.6,paste("oral baits not \ncost effective",sprintf('\u2192')))),
    key.title = title(main = "vax cost\n per dog"),
    key.axes=axis(side=4,at=q[2:7],labels=m[2:7]),
    plot.axes={
      axis(2, at=c(0.2, 0.4, 0.6, 0.8), labels=c("0.2", "0.4", "0.6", "0.8"),las=1)
...
clsoln2=data.frame(cluster=ifelse(solnf$ORV_SC>0, 1, 0), NC=Nlen, ocost=plen) 
#solnf$clustapriori=ifelse(solnf$CVR_NC>0, 3,2)
              
#clsoln=data.frame(cluster=kcout$cluster, NC=Nlen, ocost=plen)
ORVclsoln=data.frame(cluster=solnf$ORV_NC, NC=Nlen, ocost=plen)
#clustsurf=matrix(as.numeric(kcout$cluster), ncol=length(NCseq), nrow=length(obs))
#solnf$ratio=solnf$CP_SC/(solnf$DD_SC+1)

ORVclustsurf=matrix(as.numeric(solnf$clustapriori), ncol=length(NCseq), nrow=length(obs))
```

```{r plotsoln}
filled.contour(obs, NCseq,ORVclustsurf,
                ylab = "% never confined dogs", xlab = "oral bait cost (USD)",
     plot.axes={
      #abline(v=3.6)
      axis(2, at=c(0.2, 0.4, 0.6, 0.8), labels=c("0.2", "0.4", "0.6", "0.8"),las=1)
...
jpeg('baseR4_figure.jpeg', width =180, 
     height = 150,
     units = 'mm', pointsize = 12,
     quality = 75,
     res = 300)
...
matplot(x=st_comply$comp,st_comply[,1:4]/(length(obs)*length(NCseq)), type="l", las=1, lwd=2, ylab="percent of solutions including \n a given strategy for SC dogs", xlab="CP compliance rate for SC dogs", ylim=c(-.1, 1.1), bty="L", col=c(1,2,3,5))
